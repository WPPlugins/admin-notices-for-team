...
class Ant_admin_notices_team_Admin {
	...
	public function render_author_meta_box( $post ) {
		...
		if ( $users && $options['toggle-target-authors'] == 'enabled' ) { ?>
            ...
            <select name="target_author_field">
				...
				<?php foreach ( $users as $user ) { ?>
                    <option value="<?php echo $user->user_nicename; ?>"<?php selected( $user->user_nicename, $value, true ); ?>><?php echo $user->user_nicename; ?></option>
				<?php } ?>
            </select><br>
			<?php
		}
		...
		if ( $options['toggle-target-author-groups'] == 'enabled' ) {
            ...
            <select name="target_author_groups">
				...
				<?php foreach ( $groups as $group ) { ?>
                    <option value="<?php echo $group; ?>"<?php selected( $group, $value_groups, true ); ?>><?php echo $group; ?></option>
				<?php } ?>
            </select><br>
            <a href="#" id="add_group">Add Group</a><br>
            <label for="group_name" id="groupForm" class="meta_hidden">
				<?php _e( 'Group Name', 'your-plugin-textdomain' ); ?>
            </label>
            <input type="text" name="group_name" id="group_name" class="meta_hidden" value=""><br>
            <label for="add_to_group" class="meta_hidden">
				<?php _e( 'Add Authors to Group', 'your-plugin-textdomain' ); ?>
            </label>
            <input type="text" name="add_to_group" id="add_to_group" class="meta_hidden"><br>
			<?php
		}
        ...
        <input type="text" name="expiration_date" id="ant_expires" class="ant-date-picker" value="<?php echo $value_date; ?>">
		<?php


		// now get selected user id from $_POST to use in your next function
		//$user_id  = $_POST['target_author_field'];
		//$group_id = $_POST['target_author_groups'];
		//$date_id  = $_POST['expiration_date'];

		// get user data
		//$user_data        = get_user_by( 'id', $user_id );
		//$user_data_groups = get_user_by( 'id', $group_id );
		//$date_data        = get_user_by( 'id', $date_id );
	...
	public function register_settings() {
		...
		add_settings_field(
			'notice-number',
			__( '<div class="woo_box_title">Notice Number</div>', 'your-plugin-textdomain' ),
	...
	public function sandbox_register_setting( $input ) {
		...
		/*if ( isset( $input['toggle-target-authors'] ) ) {
			$options_array['toggle-target-authors'] = filter_var( $input['toggle-target-authors'], FILTER_SANITIZE_NUMBER_INT );
		} else $options_array['toggle-target-authors'] = 0;
		...
		} else $options_array['toggle-target-author-groups'] = 0;*/
		if ( isset( $input['toggle-target-authors'] ) ) {
	...
	public function sandbox_add_settings_field_single_checkbox( $args ) {
		...
		/*if ( $field_id == 'toggle-custom-style' ) {
		    $id = "customStyle";
		    } else if ($field_id == 'font-style') {
		     $id = "fontStyle";
		     } else {
		    $id = '';
        }*/
	...
	public function set_transient() {
		set_transient( "ant-admin-trans", "alive", 3 );
    }
	...
	public function notices_showing( ) {
		...
		if ( $query->have_posts() ) {
			...
			foreach ( $query->get_posts() as $notice ) {
				...
				if (
				        ($authorOptions == '' || $authorOptions == $current_user->user_login)
	...
	function ant_notice_is_expired( $notice_id = 0 ) {
     ...
     public function custom_css() {
		     ...
		     .ant-notice-dismiss  {
			     position: relative !important;
                        ...
                        left: 5% !important
                         ...
                         if ($options_design['display-inline'] == 'enabled' && $options_design['toggle-custom-style'] == 'enabled') {
                             $custom_css = "
                             .ant-notice-wrap {
                            display: flex;
                         }
	    ...
	    // if ( $authorOptions != $current_user->user_login ) {
	     /*$custom_css .= "
                 div#notice{$notice_id}  {
